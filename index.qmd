...
Hver lov og forskrift har sin egen Atom-feed. Velg dine egne, abonner i Feedly, Slack, Microsoft Teams, eller GitHub Actions — og få beskjed når Lovdata publiserer en endring.
...
- Hver endring er deep-linket til den endrede paragrafen, og inneholder et `<category>`-tag du kan filtrere på

## 📥 Bulk-data

For nedstrøms automatisering (datavarehus, compliance-dashboards, interne CDC-pipelines):

- [`amendments.jsonl.gz`](amendments.jsonl.gz) — ~91 000 rader, én per (lov, paragraf, endring). Inkluderer den nye paragraftekst (`new_text`)
- [`amendment-acts.jsonl.gz`](amendment-acts.jsonl.gz) — ~38 000 rader, én per endringslov
- [`laws.json`](laws.json) — alle 4 205 lover/forskrifter med metadata
